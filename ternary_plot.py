...
def create_ternary_contour(
    ...
    """
    def categorize_z(z_value,valor_comp):
        def generar_lista_letras(numero):
            letras = []
            for i in range(numero):
                letra = chr(65 + (i % 26))  # 65 es el código ASCII de 'A'
                letras.append(letra)
            return letras
        numero = len(valor_comp)
        lista_letras = generar_lista_letras(numero)
        for i in range(len(valor_comp)):
            if z_value == valor_comp[i]:
                return lista_letras[i] 
    ...
    z_categories = [categorize_z(z,valor_comp) for z in values]  # Aplicamos la categorización
    ...
    hovertemplate = (
        ...
        + pole_labels[3]
        + ": %{marker.color:.3f}<br>"
        + "Escenario: %{customdata}<extra></extra>"
    )
